...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $remarks = $_POST['rejectionRemarks'];
    $rejected_by = $_SESSION['srno'];
    ...
    if ($document_id) {
        ...
        if (mysqli_num_rows($checkResult) > 0) {
            $updateSql = "update document_approvals set status = 'Rejected', approved_by = $rejected_by where document_id = $document_id";
            $result = mysqli_query($conn, $updateSql);

            $auditSql = "insert into audit_logs (user_id, action, details) values ('$rejected_by', 'Rejected', '$remarks')";
